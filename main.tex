...
Native audio recordings are gathered directly within the respective linguistic regions. While we do not operate a large-scale deployment program, we conduct targeted field visits to identify fluent speakers. During these visits, data is collected using custom KoboToolbox digital forms deployed on mobile devices~\cite{kobotoolbox2024}. We designed these forms to sequentially guide the interaction: the interface first prompts the fieldworker to enter demographic metadata (such as the speaker's age, sex, and dialect variant) and log consent. It then displays the specific target word or phrase from our lexicon, prompting the fieldworker to capture the speaker's audio directly within the form using the device's microphone. This workflow automatically binds the audio file to its precise textual and demographic metadata, eliminating manual alignment errors. Because KoboToolbox functions fully offline, these structured records are securely saved locally on the device and synced in bulk to our central repository once an internet connection is available.
